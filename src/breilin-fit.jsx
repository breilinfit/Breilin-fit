...
function ChatSection() {
  ...
  const initChat = async () => {
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({ model:"claude-sonnet-4-5", max_tokens:800, system:COACH_PROMPT,
  ...
  const send = async () => {
    ...
    try {
      const res = await fetch("/api/chat", {
        method:"POST", headers:{ "Content-Type":"application/json" },
        body: JSON.stringify({ model:"claude-sonnet-4-5", max_tokens:800, system:COACH_PROMPT, messages:history }),
      });
      const d = await res.json();
